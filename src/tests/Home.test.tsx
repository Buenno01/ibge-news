...
describe('Home', () => {
  ...
  describe('NewsList', () => {
    it('should render the NewsList component', async () => {
      mockEndpoints();
      renderWithRouter(<Home />);
    ...
    it('should render the NewsList with a loading status if it\'s fetching', () => {
      mockEndpoints();
      const getLoadingStatus = () => screen.getAllByTestId('news-list-loading');

      renderWithRouter(<Home />);

      const loadingStatus = getLoadingStatus();
      loadingStatus.forEach((loadingCard) => {
